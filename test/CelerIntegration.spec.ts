...
import {
    deployMessageContracts as deployMessageContracts,
    getAccounts,
    loadFixture
} from './lib/common';
...
function computeId(sender: string, srcChainId: number, dstChainId: number, message: string) {
    return keccak256(
        ...
        [sender, srcChainId, dstChainId, message]
    );
}

function computeDirectSwapId(
    sender: string,
    srcChainId: number,
    receiver: Address,
    nonce: number,
...
function encodeMessage(
    ...
    const encoded = ethers.utils.defaultAbiCoder.encode(
        ...
        [
            [
                [dstSwap.path, dstSwap.dex, dstSwap.deadline, dstSwap.minRecvAmt],
                receiver,
                nonce,
                nativeOut
            ]
...
describe('Test transferWithSwap', function () {
    ...
    it('should directly swap', async function () {
        ...
        const expectId = computeDirectSwapId(
            sender.address,
            srcChainId,
            receiver.address,
            expectNonce,
...
describe('Test executeMessageWithTransfer', function () {
    ...
    it('should send bridge token to receiver if swap fails on dst chain', async function () {
        ...
        await expect(tx)
            .to.emit(xswap, 'SwapRequestDone')
            .withArgs(expectId, slip(amountIn, 5), expectStatus);
